                                ...
                                <tr v-for="(transaction, i) in transactions.data" :key="i">
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            >Edit</router-link>
                                            <button class="btn btn-sm btn-outline-danger" @click.prevent="destroy(transaction.id, i)">Delete</button>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
...
export default {
    setup() {
        ...
        function destroy(id, i) {
            axios.delete(
                `http://127.0.0.1:8000/api/transaction/${id}`,
            )
            .then(() => {
                transactions.value.data.splice(i, 1)
            }).catch((err) => {
